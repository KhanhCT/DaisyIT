package edu.c9.lab411.googlemap;

import java.io.InputStream;
import java.net.HttpURLConnection;
import java.net.URL;

import com.google.android.gms.maps.GoogleMap;
import com.google.android.gms.maps.model.Marker;

import android.app.Activity;
import android.graphics.Bitmap;
import android.graphics.BitmapFactory;
import android.os.AsyncTask;

public class ImageLoadTask extends AsyncTask<Void, Void, Bitmap> {

	//Link url hình ảnh bất kỳ
    private String url;
    private GoogleMap map;
    private Activity context;
    private boolean isCompleted=false;
    private Marker currentMarker;
	private String PLoca_ID_, PLoca_Code_, Partner_ID_, PLoca_Type_,PLoca_Name_, PLoca_Address_, Place_ID_;

    public boolean isCompleted() {
		return isCompleted;
	}

	public void setCompleted(boolean isCompleted) {
		this.isCompleted = isCompleted;
	}

	public ImageLoadTask(Activity context, String url,GoogleMap map,Marker currentMarker,  String PLoca_ID,
			String PLoca_Code, String Partner_ID, String PLoca_Type,
			String PLoca_Name, String PLoca_Address, String Place_ID) {
        this.context=context;
		this.url = url;
        this.map=map;
        this.currentMarker=currentMarker;
		this.PLoca_ID_ = PLoca_ID;
		this.PLoca_Code_ = PLoca_Code;
		this.Partner_ID_ = Partner_ID;
		this.PLoca_Type_ = PLoca_Type;
		this.PLoca_Name_ = PLoca_Name;
		this.PLoca_Address_ = PLoca_Address;
		this.Place_ID_ = Place_ID;
    }

    @Override
    protected Bitmap doInBackground(Void... params) {
        try {
        	//Tiến hành tạo đối tượng URL
            URL urlConnection = new URL(url);
            //Mở kết nối
            HttpURLConnection connection = (HttpURLConnection) urlConnection
                    .openConnection();
            connection.setDoInput(true);
            connection.connect();
            //Đọc dữ liệu
            InputStream input = connection.getInputStream();
            //Tiến hành convert qua hình ảnh
            Bitmap myBitmap = BitmapFactory.decodeStream(input);
            if(myBitmap==null)
            	return null;
            return myBitmap;
        } catch (Exception e)
        {
            e.printStackTrace();
        }
        return null;
    }

    @Override
    protected void onPostExecute(Bitmap result) {
        super.onPostExecute(result);
        //thiết lập Info cho Map khi tải hình hoàn tất
        map.setInfoWindowAdapter(new MyInfoWindowAdapter(context,result, PLoca_ID_, PLoca_Code_, Partner_ID_, PLoca_Type_,PLoca_Name_, PLoca_Address_, Place_ID_));
        //tiến hành hiển thị lên Custom marker option lên Map:
        currentMarker.showInfoWindow();
    }
}
